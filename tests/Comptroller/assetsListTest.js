...
//     });
    ...
    it.skip("adds to the asset list only once", async () => {
    ...
    it.skip("the market must be listed for add to succeed", async () => {
    ...
    it.skip("returns a list of codes mapping to user's ultimate membership in given addresses", async () => {
  ...
  describe('exitMarket', () => {
    ...
    it.skip("accepts when you're not in the market already", async () => {
      await enterAndCheckMarkets([OMG, BAT], [OMG, BAT]);
    ...
    it.skip("properly removes when there's only one asset", async () => {
      await enterAndCheckMarkets([OMG], [OMG]);
      await exitAndCheckMarkets(OMG, [], 'NO_ERROR');
    });

    it.skip("properly removes when there's only two assets, removing the first", async () => {
      await enterAndCheckMarkets([OMG, BAT], [OMG, BAT]);
      await exitAndCheckMarkets(OMG, [BAT], 'NO_ERROR');
    });

    it.skip("properly removes when there's only two assets, removing the second", async () => {
      await enterAndCheckMarkets([OMG, BAT], [OMG, BAT]);
      await exitAndCheckMarkets(BAT, [OMG], 'NO_ERROR');
    });

    it.skip("properly removes when there's only three assets, removing the first", async () => {
      await enterAndCheckMarkets([OMG, BAT, ZRX], [OMG, BAT, ZRX]);
      await exitAndCheckMarkets(OMG, [ZRX, BAT], 'NO_ERROR');
    });

    it.skip("properly removes when there's only three assets, removing the second", async () => {
      await enterAndCheckMarkets([OMG, BAT, ZRX], [OMG, BAT, ZRX]);
      await exitAndCheckMarkets(BAT, [OMG, ZRX], 'NO_ERROR');
    });

    it.skip("properly removes when there's only three assets, removing the third", async () => {
      await enterAndCheckMarkets([OMG, BAT, ZRX], [OMG, BAT, ZRX]);
      await exitAndCheckMarkets(ZRX, [OMG, BAT], 'NO_ERROR');
    });
  });
  ...
  describe('entering from borrowAllowed', () => {
    it.skip("enters when called by a ctoken", async () => {
    ...
    it.skip("reverts when called by not a ctoken", async () => {
      await expect(
        send(comptroller, 'borrowAllowed', [BAT._address, customer, 1], {from: customer})
      ).rejects.toRevertLive('Returned error: execution reverted: sender must be cToken');
