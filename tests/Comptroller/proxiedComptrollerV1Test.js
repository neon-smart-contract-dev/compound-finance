...
describe('ComptrollerV1', function() {
  ...
  describe('delegating to comptroller v1', () => {
    ...
    describe('becoming brains sets initial state', () => {
      it.skip('reverts if this is not the pending implementation', async () => {
        await expect(
          send(brains, '_become', [unitroller._address, oracle._address, 0, 10, false])
        ).rejects.toRevertLive('Returned error: execution reverted: change not authorized');
      });

      it.skip('on success it sets admin to caller of constructor', async () => {
      ...
      it.skip('on success it sets closeFactor and maxAssets as specified', async () => {
      ...
      it.skip("on reinitialization success, it doesn't set closeFactor or maxAssets", async () => {
    ...
    describe('_setCollateralFactor', () => {
      ...
      it.skip('fails if not called by admin', async () => {
        expect(
      ...
      it.skip('fails if asset is not listed', async () => {
        expect(
          await send(unitrollerAsComptroller, '_setCollateralFactor', [cToken._address, half])
        ).toHaveTrollFailure('MARKET_NOT_LISTED', 'SET_COLLATERAL_FACTOR_NO_EXISTS');
      });

      it.skip('fails if factor is too high', async () => {
      ...
      it.skip('fails if factor is set without an underlying price', async () => {
        const cToken = await makeCToken({ supportMarket: true, comptroller: unitrollerAsComptroller });
        expect(
          await send(unitrollerAsComptroller, '_setCollateralFactor', [cToken._address, half])
      ...
      it.skip('succeeds and sets market', async () => {
        const cToken = await makeCToken({ supportMarket: true, comptroller: unitrollerAsComptroller });
    ...
    describe('_supportMarket', () => {
      ...
      it.skip('fails if asset is not a CToken', async () => {
        const notACToken = await makePriceOracle();
        await expect(send(unitrollerAsComptroller, '_supportMarket', [notACToken._address])).rejects.toRevertLive("Returned error: execution reverted");
      });
      ...
      it.skip('cannot list a market a second time', async () => {
      ...
      it.skip('can list two different markets', async () => {
