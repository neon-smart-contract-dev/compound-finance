    ...
    describe(kind, () => {
      ...
      if (kind == 'jump-rate') {
        ...
        it('handles overflowed cash + borrows', async () => {
          await expect(getBorrowRate(model, UInt256Max(), UInt256Max(), 0)).rejects.toRevertLive("Returned error: execution reverted");
        });

        it('handles failing to get exp of borrows / cash + borrows', async () => {
          await expect(getBorrowRate(model, 0, UInt256Max(), 0)).rejects.toRevertLive("Returned error: execution reverted");
        });
        ...
        it('handles overflow utilization rate times slope', async () => {
          ...
          await expect(getBorrowRate(badModel, 1, 1, 0)).rejects.toRevertLive("Returned error: execution reverted");
        });
        ...
        it('handles overflow utilization rate times slope + base', async () => {
          ...
          await expect(getBorrowRate(badModel, 0, 1, 0)).rejects.toRevertLive("Returned error: execution reverted");
        });
      }
    });
