...
describe('CCompLikeDelegate', function () {
  describe("_delegateCompLikeTo", () => {
    it("does not delegate if not the admin", async () => {
      ...
      await expect(send(cToken, '_delegateCompLikeTo', [a1], {from: a1})).rejects.toRevertLive('Returned error: execution reverted: only the admin may set the comp-like delegate');
    });
